...
namespace appify.web.api.Controllers
{
    ...
    public async Task<IActionResult> GetMemberProductsList(ParamMemberUserID itemData)
    {
        ...
        [HttpPost]
        [Route("productlistisnewarrival")]
        [MapToApiVersion("1.0")]
        public async Task<IActionResult> GetMemberProductsListByIsNewArrival(ParamProductsByArrival itemData)
        {
            var reqHeader = Request;
            string controllerURL = new Uri(HttpContext.Request.GetDisplayUrl()).AbsoluteUri;
            try
            {
                ...
                List<MemberProduct> items = customerBusiness.ProductListIsNewArrival(itemData);
                if (items?.Any() == true)
                {
                    rm.statusCode = StatusCodes.OK;
                    rm.message = "FETCH PRODUCT LIST";
